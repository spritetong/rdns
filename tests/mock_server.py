...
async def record_requests(request: Request, call_next):
    ...
    if not request.url.path.startswith("/api/records") and not request.url.path.startswith("/api/reset"):
...
@app.get("/nic/update")
def mock_dynu_update(hostname: str = "", myip: str = "", myipv6: str = "", password: str = ""):
