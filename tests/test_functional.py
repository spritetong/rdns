...
def get_binary_path():
    ...
    if not bin_path.exists():
        raise FileNotFoundError(f"RDNS binary not found at {bin_path}. Run 'cargo build' first.")
    return bin_path
...
def run_tests():
    ...
    try:
        ...
        res = subprocess.run([str(binary), "--check", "-c", str(CONFIG_PATH)], capture_output=True, text=True, encoding="utf-8")
        ...
        invalid_config.write_text("global:\n  interval: 10\ntasks: []", encoding="utf-8")
        res_inv = subprocess.run([str(binary), "--check", "-c", str(invalid_config)], capture_output=True, text=True, encoding="utf-8")
        ...
        reset_server()
        res = subprocess.run([str(binary), "--dry-run", "-c", str(CONFIG_PATH)], capture_output=True, text=True, encoding="utf-8")
        assert res.returncode == 0, f"--dry-run failed: {res.stderr}"
        assert "[DRY-RUN PREVIEW: mock-dualstack]" in res.stdout, "Preview header missing"
        assert "mocksecret" not in res.stdout, "Secret token should be masked"

        records = get_records()
        write_requests = [r for r in records if r["path"] in ["/nic/update", "/api/v1/ddns/update", "/api/v1/notify"]]
        assert len(write_requests) == 0, f"Dry-run should not send write requests, found: {write_requests}"
        ...
        reset_server()
        res = subprocess.run([str(binary), "--once", "-c", str(CONFIG_PATH)], capture_output=True, text=True, encoding="utf-8")
        ...
        assert "myipv6=2001%3Adb8%3A%3A1234%3A5678" in nic_updates[0]["query"] or "2001:db8::1234:5678" in nic_updates[0]["query"]

        assert len(webhook_updates) == 1, f"Expected 1 /api/v1/ddns/update, got {len(webhook_updates)}"
        ...
        assert "Bearer tok123" in auth_hdr, f"Expected Authorization header with env expansion, got: {auth_hdr}"

        assert len(notifications) >= 1, f"Expected notification webhook, got {len(notifications)}"
        ...
        print("  [PASS] Single run live update, templating, and atomic state write succeeded")
        ...
        records = get_records()
        write_requests = [r for r in records if r["path"] in ["/nic/update", "/api/v1/ddns/update"]]
        assert len(write_requests) == 0, f"Unchanged IP should not trigger updates, got: {write_requests}"
        ...
        res_t1 = subprocess.run([str(binary), "--once", "-c", str(CONFIG_PATH), "-t", "1"], capture_output=True, text=True, encoding="utf-8")
        assert res_t1.returncode == 0
        res_t4 = subprocess.run([str(binary), "--once", "-c", str(CONFIG_PATH), "--worker-threads", "4"], capture_output=True, text=True, encoding="utf-8")
